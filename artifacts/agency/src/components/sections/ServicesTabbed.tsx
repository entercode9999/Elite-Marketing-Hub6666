...
function subViz(
  ...
  return (
    <div className="h-full flex flex-col gap-3 justify-center p-4">
      <div className="bg-white rounded-xl shadow-md p-3.5 border border-gray-100">
        <p className="text-[9px] font-black uppercase tracking-widest text-gray-300 mb-3">{title}</p>
        {metrics.map((m) => (
          <div key={m.l} className="flex items-center justify-between py-1.5 border-b border-gray-50 last:border-0">
            <span className="text-[10px] text-gray-500">{m.l}</span>
            <span className="text-[10px] font-black" style={{ color: m.c ?? color }}>{m.v}</span>
          </div>
        ))}
      </div>
