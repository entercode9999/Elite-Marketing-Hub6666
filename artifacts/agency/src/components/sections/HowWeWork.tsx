        ...
        <div className="space-y-0 border border-[#e5e2d9] rounded-2xl overflow-hidden">
          {PHASES.map((phase, i) => (
            <motion.div
              ...
              className={`grid grid-cols-1 md:grid-cols-[72px_1fr] gap-6 p-7 border-b border-[#e5e2d9] last:border-0 ${
                i % 2 === 0 ? "bg-white" : "bg-[#f9f8f5]"
              }`}
            >
              {/* Number */}
              <div className="flex items-start pt-0.5">
                <span className="text-3xl font-black text-primary/20 leading-none">{phase.number}</span>
              </div>
              {/* Content */}
              <div>
                <p className="font-black text-xs text-primary uppercase tracking-widest mb-2">{phase.label}</p>
                <h3 className="font-black text-base text-[#0e0e0e] mb-2.5 leading-snug">{phase.heading}</h3>
